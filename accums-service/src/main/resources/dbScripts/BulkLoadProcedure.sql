CREATE OR REPLACE PROCEDURE LOAD_ACCUM_UTILIZATIONS (in ID int)
...
BEGIN
	...
	( VALUES(1) UNION ALL
	SELECT LDGR_ID + 1 FROM LDGR_IDS WHERE LDGR_ID < LIMIT )
	...
	WHILE  C <= 3 DO
        ...
        VALUES (SEQ_LDGR_ENTRY.nextval,LINK,V.LDGR_ID,CONCAT('ACCUM',cast(ACCUM_COUNTER as char(7))),'PTNT','PPO',V.ALWD_AMT,V.NTWK_CD,V.ALWD_AMT,V.UOM_NM,V.SVC_DT);
			...
			IF (AMOUNT  IS NULL) THEN
				...
				values(SEQ_LDGR_SUM.nextval,CONCAT('ACCUM',cast(ACCUM_COUNTER as char(7))),V.MBR_ID,V.NTWK_CD,V.NTWK_TIER_NM,CURRENT TIMESTAMP, CURRENT DATE,V.PLN_ID,V.SUB_ID,V.ALWD_AMT,1000,100,V.UOM_NM);
			ELSE
